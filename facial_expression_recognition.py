...
def setup_kaggle_script():
    script_name = 'facial_expression_recognition.py'
    dest = f'/kaggle/working/{script_name}'
    if os.path.exists(dest):
        return dest
    for root, _, files in os.walk('/kaggle/input'):
        if script_name in files:
            source = os.path.join(root, script_name)
            shutil.copy(source, dest)
            print(f'Copied script from: {source}')
            return dest
    raise FileNotFoundError(
        'facial_expression_recognition.py not found. '
        'Add it via Add Data -> Upload, or paste the file into /kaggle/working/'
    )
    ...
    for root in search_roots:
        ...
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            if 'train' in dirnames:
                ...
                if os.path.isdir(train_path):
                    subdirs = [
                        d.lower() for d in os.listdir(train_path)
...
def visualize_gradcam_samples(model, X_test, y_test, model_name, last_conv_layer_name, num_samples=5, save_dir=None):
    ...
    for i, idx in enumerate(indices):
        try:
            ...
            pred_label = Config.EMOTIONS[np.argmax(pred)]
...
def evaluate_model(model, X_test, y_test, y_test_cat, model_name):
    ...
    report_path = os.path.join(
        Config.OUTPUT_DIR, f'{model_name.replace(" ", "_").lower()}_classification_report.txt'
    )
...
def run_pipeline(data_source='csv', csv_path=None, data_dir=None):
    ...
    if data_source == 'csv' and csv_path:
        ...
        X_train, y_train, X_val, y_val, X_test, y_test = load_from_directory(
            data_dir, quick_demo=Config.QUICK_DEMO
        )
    else:
    ...
    if Config.QUICK_DEMO and data_source == 'csv':
        print(f"Demo mode: {Config.DEMO_SAMPLES_PER_CLASS} samples per class, {epochs} epochs")
    ...
    try:
        last_conv_layer = get_last_conv_layer_name(custom_cnn)
        visualize_gradcam_samples(custom_cnn, X_test, y_test, 'CustomCNN', last_conv_layer, num_samples=5, save_dir=os.path.join(Config.OUTPUT_DIR, 'gradcam_cnn'))
    except Exception as e:
    ...
    history_vgg_phase1 = vgg_model.fit(
        train_gen_rgb, epochs=phase1_epochs, validation_data=val_gen_rgb,
    ...
    history_vgg_phase2 = vgg_model.fit(
        train_gen_rgb, epochs=phase2_epochs, validation_data=val_gen_rgb,
...
if __name__ == "__main__":
    ...
    if args.csv and os.path.exists(args.csv):
        ...
        if csv_path:
            ...
            print("Add FER2013 on Kaggle: Add Data -> search msambare/fer2013")
            print("Or download: https://www.kaggle.com/datasets/msambare/fer2013")
